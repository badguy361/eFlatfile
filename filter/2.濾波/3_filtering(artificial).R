...
### LOAD Functions
###
setwd("C:/Users/user/Desktop/Central/internship/3-Filtering/") 
source("SGM_Process/Function_Integrate2VD.r")    #積???函??????
source("SGM_Process/Function_Integrate_Ia.r")    #Ia計???
...
source("SGM_Process/Function_write_File.r")      #寫入檔???
source("SGM_Process/Function_write_data.r")      #寫??????
...
ProcessTH_art <- function(filter.ID, author, Baseline=TRUE, PreBaseline=FALSE, Skip=0, Add=20, Taper=0, nDC=2000, tb=5, te=5, nPole=2.5) {
  ...
  rpage <- as.character(paste0("C:/Users/user/Desktop/Central/internship/events_2020/",str_pad(data.info$Month,2, pad = "0"),"/",data.info$ts_name)) 
  ...
  ifelse(file.exists("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/PROCESS_FILE_tmp"),paste("file exit!"),dir.create("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/PROCESS_FILE_tmp"))
  path1 <- paste("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/PROCESS_FILE_tmp/",YEAR,sep="")
  ifelse(file.exists(path1),paste("file exit!"),dir.create(path1))
  path2 <- paste("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/PROCESS_FILE_tmp/",YEAR,"/",EQ_ID,sep="")
  ifelse(file.exists(path2),paste("file exit!"),dir.create(path2))
  #CHECK存放?????????置??????存在
  # path8 <- paste("2019濾波_SSHAC/PROCESS_FILE_??????/",YEAR,"/",EQ_ID,"/PIC",sep="")
  # ifelse(file.exists(path8),paste("file exit!"),dir.create(path8))
  
  # 測試??????已???在濾波檔??????若已???濾???波，???跳???function
  # if(file.exists(paste("D:/2019濾波_SSHAC/PROCESS_FILE_??????_H1_NS/TXT/",SGM_ID,"_",SGM_SN,".txt",sep=""))){
  ...
  for (i in 1:3){
    ...
    # filtered by human
    fc <- c()
    # hpv <- read.table(paste0("C:/Users/user/Desktop/3-Filtering/Filtering Result/TXT/",SGM_ID,"_",filter.ID,".txt"),sep=",",header = T)
    ...
    fs.amp_o <- abs(fs[1:nNyq])    # fs.amp[-1] : ???fs.amp第??????數字拿???
    ...
    if(i ==2){
      NS <- c(dt,acc.flt)
      write.table(NS,file=paste0("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/4-RCTC/RCTC Result/Tmp save file/NS.txt"),col.names = FALSE,row.names = FALSE)
    } else if (i==3) {
      EW <- c(dt,acc.flt)
      write.table(EW,file=paste0("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/4-RCTC/RCTC Result/Tmp save file/EW.txt"),col.names = FALSE,row.names = FALSE)
    }
    nametransfer(filedir1 = paste0("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/4-RCTC/RCTC Result/Tmp save file/NS.txt"), filedir2 =paste0("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/4-RCTC/RCTC Result/Tmp save file/EW.txt"), stationname = SGM_SN , sn = SGM_ID,
                 outputdir = 'C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/4-RCTC/RCTC Result/Inputdata/')
  ...
  #CHECK存放濾波???段????????????置??????存在
  path9 <- paste("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/TXT_2020",sep="")
  ...
  #  write.table(yy, file=paste("D:/2019濾波_SSHAC/RSP/",SGM_SN,"_RSP.txt",sep=""),sep = " ",row.names = F)  
  ...
  path11 <- paste("C:/Users/user/Desktop/Central/internship/3-Filtering/Filtering Result/RSP_2020/",sep="")
...
# # # 
# for(i in 701:707){
#   kk <- str_pad(i,3, pad = "0")
#   filter.ID <- paste0("B000",kk)
#   ProcessTH_art(filter.ID, "yang", Baseline=TRUE, PreBaseline=FALSE, Skip=0, Add=20, Taper=0, nDC=2000, tb=5, te=5, nPole=2.5)
# }
#  
# jjj <- data.frame()
# jjj <- list.files("C:/Users/user/Desktop/3-Filtering/Filtering Result/TXT/")
