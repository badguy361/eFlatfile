...
ProcessTH_art <- function(filter.ID, author, Baseline=TRUE, PreBaseline=FALSE, Skip=0, Add=20, Taper=0, nDC=2000, tb=5, te=5, nPole=2.5) {
  ...
  #
  # Hypo <- round(data.info$Hypo,2) #Ë∑üRrup‰∏ÄÊ®?(??õÊç®‰∫îÂÖ•‰ΩçÁΩÆ‰∏ç‰?ÄÊ®?)
  ...
  # path8 <- paste("2017ÊøæÊ≥¢_SSHAC/PROCESS_FILE_??????/",YEAR,"/",EQ_ID,"/PIC",sep="")
  # ifelse(file.exists(path8),paste("file exit!"),dir.create(path8))
  
  
  # if(file.exists(paste("D:/2017ÊøæÊ≥¢_SSHAC/PROCESS_FILE_??????_H1_NS/TXT/",SGM_ID,"_",SGM_SN,".txt",sep=""))){
  #   print(paste(SGM_SN,"has already done!"))
  #   return()
  # }
  
  
  acc.v <- accs$Z/978.88 #Ê≠?Ë¶èÂ?ñÁ?ÑÊ?ÇÂøµ?
  acc.h1 <- accs$H1/978.88
  acc.h2 <- accs$H2/978.88
  #  x <- seq(1:dim(acc)[1])*data$DT
  npts<-dim(accs)[1] #dimÂæóÂà∞Ë©≤table raw?ï∏ -> Á∏ΩÂÖ±??âÂ?öÂ?ëÂÄãÈ??(?åØÂπ?)
  start.time <- 0.0
  ...
  for (i in 1:3){
    ...
    #
    if (nDC < 0) 	acc <- acc - mean(acc[1:npts]) else 	acc <- acc - mean(acc[1:nDC]) #?ü∫Á∑öÊ†°Ê≠???ÑÊ?õÂ??2000ÈªûÂπ≥???
    ...
    if (Taper > 0) nTaper <- npts * Taper / 100 else nTaper <- Taper
    nSkip <- ceiling(Skip/dt) #ceiling??õÊç®‰∫îÂÖ• skip?
    nAdd  <- ceiling(Add/dt) # add?
    acc.bs  <- BslnAdj(acc, nTaper, nSkip, nAdd) #‰ª•Ê?ÄÂ∞èÂπ≥?ñπÊ≥ïÊì¨??àÂá∫‰ΩçÁßªÊ≠∑Ê?Ç‰?ãÂü∫Á∑?
    ...
    plot(acc, type="l", ylab="Acceleration (g)", xlab="time (sec)") #Ë∑ëÊ?âÁ?ÑÁ??
    lines(acc.bs, col=6) #col -> pink ?ü∫Á∑öÊ†°Ê≠???éÂ?åÁ?ÑÁ??
    abline(h=0) #y = 0?ï´‰∏ÄÊ¢ùÁ??
    ...
    CairoPNG(filename = path.1, width = 1280, height = 1024 ,pointsize = 24, bg = "white") #Â≠òÊ?îÊ??(bc)(?î®CairoPNGÂ≠òÊ?îË?ÉÂ•Ω???)
    ...
    text(x = 0.1, y = -0.8, text2, cex = 1.5, col = "blue",font=1) #Mw Rrup??ÑÂ?óÈ?îÈ?èËâ≤Â§ßÂ??..
    #... 1. Acceleration time history
    par(mar = c(4,4,4,0)) 
    ...
    rsp <- spectraw2(acc.bs, 0.05, 'psv') # spectraw2(acc.bs, damping, 'psv') ?ï´??çÊ?âË?úÂ?öÊøæÊ≥?
    ...
    # filtered by human
    fc <- c()
    tmp <- locator(n=1, type='p', pch=16, col=6) #locator??ãÈ?ûÁ?Ñ‰?çÁΩÆÂ∫ßÊ??
    fc[1] <- 1./tmp$x[1] #ÂæóÂÖ∂?†ª???(?Ä±Ê?üÂÄíÊï∏)
    fc[2] <- 100
    # print(c(fc[1]))
    # print(c(fc[2]))
    
    # ??öÂ?ÖÁ?ãË?âË?âÊ??(Ë¶ÅÊøæÊ≥¢‰??)
    ...
    fs.amp_o <- abs(fs[1:nNyq])    # fs.amp[-1] : ???fs.ampÁ¨????????ï∏Â≠óÊãø???
    ...
    abline(v=array(outer(1:9, c(0.01,0.1,1,10,100))), col=8, lty=2) #Ê®ôÂá∫?ç≥Â∞áhighpass??êÊ≥¢ÊÆµÁ?ÑÊ≥¢
    ...
    CairoPNG(filename = path.4, width = 1280, height = 1024 ,pointsize = 24, bg = "white") #Â≠òÂ?ÖÁ?ãË?âË?âÊ?õÁ?ÑÂ??(FFT)
    plot(c(0.01,100), range(fs.amp[-1]), log='xy', type='n',xlab='Frequency (Hz)', ylab='Fourier Amplitude')
    ...
    #
    fs[1] <- complex(real=Re(fs[1]), imaginary=0) #complex??õÊï∏(?õÆ??çË?õÊï∏=0)
    fs[nNyq] <- complex(real=Re(fs[nNyq]), imaginary=0)
    fs[nfft+2-(2:(nfft/2))] <- Conj(fs[2:(nfft/2)]) #conj?Ö±ËªõË?áÊï∏
    
    #	
    #... Inverse FF 
    #
    acc.flt <- ts(data=Re(fft(fs, inverse=T))/nfft, deltat=dt, start=start.time) #??çÂ?ö‰?ÄÊ¨°Â?ÖÁ?ãË?âË?âÊ?õÂ?ûÊ?ÇÈ?ìÂ?? ReÂæóÂà∞Ë©≤Ë?áÊï∏??ÑÂØ¶?ï∏
    
    #
    #... Keep only the original length
    #  
    acc.flt <- ts(acc.flt[1:npts], deltat=dt, start=start.time) #ËÆäÊ?êÂ?üÊú¨??ÑdtÁ∏ΩÊï∏
    
    #	
    #... Correct baseline drift after filtering	
    #
    if (Baseline) acc.flt <- BslnAdj(acc.flt, nTaper, nSkip, nAdd) #??çÈÄ≤Ë?å‰?ÄÊ¨°Âü∫Á∑öÊ†°Ê≠?
    ...
    layout(matrix(1:3,3,1))
    #... 1. Acceleration time history
    plot(acc.bs, type="l", ylab="Acceleration (g)", xlab="time (sec)") #?ï´?ü∫Á∑öÊ†°Ê≠?+ÊøæÊ≥¢+??çÂü∫Á∑öÊ†°Ê≠?ÂæåÁ?ÑÂ??
    lines(acc.flt, col=6)
    abline(h=0)
    ...
    CairoPNG(filename = path.2, width = 1280, height = 1024 ,pointsize = 24, bg = "white") #??äÂ?ñÂ?òËµ∑‰æ?(fl)
    layout(matrix(1:4,4,1), heights=c(1.5,4,4,4))
    par(mar = c(0,0,0,0))
    plot(0,0, ann = F, bty = 'n', type = 'n', xaxt = 'n', yaxt = 'n')
    ...
    windows(width = 6, height = 6)
    plot_spectra(rsp.flt, "psv") #?ï´??çÊ?âË??(psv) Á¨¨Â?õÂºµ???
    ...
    plot_spectra(rsp.flt, "psa") #?ï´PSA??çÊ?âË??
    ...
    acc.md5 <- digest(paste(acc.flt, collapse=","), serialize=FALSE) #‰∏çÈ?çË?ÅÂ?åÈù¢Ê≤íÁî®?à∞
    ...
    ### for running RCTC 
    if(i ==2){
      NS <- c(dt,acc.flt) #Ëº∏ÂÖ•ÊøæÂ?åÊ≥¢ÂæåÁ?ÑÊù±?ù±
  ...
  #  write.table(yy, file=paste("D:/2017ÊøæÊ≥¢_SSHAC/RSP/",SGM_SN,"_RSP.txt",sep=""),sep = " ",row.names = F)  
  ...
  CairoPNG(filename = path.12, width = 1280, height = 1024 ,pointsize = 24, bg = "white") #Â≠òËµ∑‰æÜÊ?ÄÂæåÁ?ÑÂ?çÊ?âË?? ?Ö∂pgaÂπ≥ÊñπÂæåÁ?ÑÁ?êÊ??
  y <- psa.sqrt
  yrange <- range(y)
