...
data.all <- read.table("Âoªi³æ/filter2016.txt",header=T)
...
ProcessTH <- function(SGM_SN, author, Baseline=TRUE, PreBaseline=FALSE, Skip=0, Add=20, Taper=0, nDC=2000, tb=5, te=5, nPole=2) {
  ...
  for (i in 1:3){
    ...
    if(i ==1) ACC.Z.bs <-acc.bs else if(i == 2) ACC.NS.bs <-acc.bs else ACC.EW.bs <- acc.bs
    ...
    path.1=paste(path8,"/",SGM_ID,".AVD_bc.",text.comp,".png",sep="" )
   ...
   #fc <- sort(fc$x)
    ...
    path.4=paste(path8,"/",SGM_ID,".FFT.",text.comp,".png",sep="")
    ...
    #
    path.2=paste(path8,"/",SGM_ID,".AVD_fl.",text.comp,".png",sep="")
    ...
    path.3=paste(path8,"/",SGM_ID,".rsp.",text.comp,".png",sep="")
  ...
  path4 <- paste("L:/±j¦a°Ê¥[­È¸ê®Æ®w/07_SGM/PROCESS_FILE/",YEAR,"/",EQ_ID,"/ACC/",SGM_ID,".IFO",sep="")
  ifelse(file.exists(path4),paste("file exit!"),file.copy(from=rpage,to=path4))
  path9 <- paste("L:/±j¦a°Ê¥[­È¸ê®Æ®w/07_SGM/PROCESS_FILE/",YEAR,"/",EQ_ID,"/ACC",sep="")
  #¿é¥X³t«×¸ê®Æ
 ...
 # path9 <- paste("C:/Users/stella/Desktop/SGMDB16/SGM/PROCESS_FILE/",YEAR,"/",EQ_ID,"/ACC",sep="")
  #¿é¥X¥u¦³°ò½u®Õ¥¿ªº¥[³t«×¸ê®Æ
  Write_data(paste(path9,"/",SGM_ID,".bs.ACC",sep=""),SGM_ID,ACC.Z.bs,ACC.NS.bs,ACC.EW.bs,npts,dt)
