...
# ProcessTH <- function(SGM_SN, author, Baseline=TRUE, PreBaseline=FALSE, Skip=0, Add=20, Taper=0, nDC=2000, tb=5, te=5, nPole=2) {
  ...
  data.info  <- filter.frame[filter.frame$filter.id==SGM_SN,] #¾É¤Jfilter.id
  rpage <- as.character(paste0("@ts data/BH_2018/",str_pad(data.info$Month,2, pad = "0"),"/",data.info$ts_name)) #¾É¤JBH2018_01 
  accs  <- read.table(rpage, skip=1, col.names=c("time","Z","H1","H2")) #Åª¤Jasc¸ê°T
  
  dt <- accs$time[2]-accs$time[1] #accsªº®É¶¡®t(dt)
  YEAR <- data.info$Year
  MON <- data.info$Month
  DAY <- data.info$Day
  HOUR <- data.info$Hour
  ...
  npts<-dim(accs)[1] #accsªºÁ`¦C¼Æ  #dim ¨ú±o¸ê®Æ¦C¦æ¼Æ
  start.time <- 0.0
...
#    text(0,max(dis.bs)*0.5,substr(paste("PGD=",max(abs(dis.bs))),1,12), pos=4)
  ...
  #   fs.amp_o <- abs(fs[1:nNyq])    # fs.amp[-1] : ??fs.amp?Ä¤@?Ó¼Æ¦r????
  ...
  #   #fc <- sort(fc$x) # locator() ?|?^???Iï¿½ï¿½??x??y?y??
  #   #fc <- c(data.info$Lowest_Usable_Freq_Ave,100) #?T?w?W?q?o?i
  #   
  #   # 2019.01.13: ?D??Period = 0.5 sec ~ 10 sec ?????Arsv?Ì§C?È¹?ï¿½ï¿½???g?ï¿½ï¿½A?Ë¼Æ±o???W?v
  #   fc <- c()
...
#   }  #end of loop 3 component
  ...
  #CHECK?s???o?i?W?q???r?É¦??m?O?_?s?b
  # path9 <- paste("F:/2019?o?i_SSHAC?H?~/PROCESS_FILE_?È©?/TXT",sep="")
