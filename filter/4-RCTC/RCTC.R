...
for( i in 1:nrow(filter.frame)){ # 
  # i <- 1
  setwd("D:/Myanmar/filter/4-RCTC")
  data.info <- filter.frame[i,]
  print(i)
  print(data.info$filter.id)
  path0 <- "D:/Myanmar/filter/4-RCTC/RCTC Result/"
  path1 <- "D:/Myanmar/filter/4-RCTC/RCTC Result/Inputdata/"
  path2 <- "D:/Myanmar/filter/4-RCTC/RCTC Result/Inputdata_For_Run/"
...
#################################
library(stringr)
setwd("D:/Myanmar/filter/4-RCTC/RCTC Result/Outputdata_gather/")
myMergedData.summary <-
  do.call(rbind,lapply(list.files(path = "D:/Myanmar/filter/4-RCTC/RCTC Result/Outputdata_gather/",pattern = ".csv"), read.csv))
