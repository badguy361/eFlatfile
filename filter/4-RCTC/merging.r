library(stringr)
result_TXT <- list.files("D:/Myanmar/filter/3-Filtering/Filtering Result/TXT_2021/")
dsheet <- read.csv("D:/Myanmar/filter/3-Filtering/sgm.2021_Frv_Fnm.rec_Rrup.csv", header = T)
# sss <- colnames(dsheet)
...
for(i in 1:length(result_TXT)){
    # i = 8
  file_TXT <- paste0("D:/Myanmar/filter/3-Filtering/Filtering Result/TXT_2021/",result_TXT[i])
...
#period
result_RSP_1 <- list.files("D:/Myanmar/filter/3-Filtering/Filtering Result/RSP_2021/")
periodlab <- array()
sqrtlab <- array()
p <- read.table(paste0("D:/Myanmar/filter/3-Filtering/Filtering Result/RSP_2021/",result_RSP_1[1]),header = T,sep="")
DVA <- c("PSD","PSV","PSA")
component <- c("NS","EW","Z")
...
#RSP
result_RSP <- list.files("D:/Myanmar/filter/3-Filtering/Filtering Result/RSP_2021/")
...
for(i in 1:length(result_TXT)){
  # i = 8 
  file_RSP <- paste0("D:/Myanmar/filter/3-Filtering/Filtering Result/RSP_2021/",TXT_fi[i],"_RSP.txt")
  nr <- nr +1
  station[nr] <- TXT_fi[i]
...
# write.table(final_result,"C:/Users/user/Desktop/_final_result.csv",col.names = T,row.names = F,sep = ",")

sheet_rot <- read.csv("D:/Myanmar/filter/4-RCTC/RCTC Result/Outputdata_gather/myMergedData.summary.csv", header = T)
...
# write.table(Result,"C:/Users/user/Desktop/rec_txt_RSP_RCTC_.csv",col.names = T,row.names = F,sep = ",")


Result$filter.id <- paste0("B000",str_pad(c(1:nrow(Result)),3,"left","0"))
write.table(Result,"D:/Myanmar/filter/4-RCTC/rec_txt_RSP_RCTC_2021.csv",col.names = T,row.names = F,sep = ",")



# 
# ###Calulate used data and graphing
# dsheet <- read.csv("C:/Users/user/Desktop/_final_result_NA.csv", header = T)
# lab <- colnames(dsheet)
...
# legend("topright",ncol = 1,col = "black",legend = " data",fill= "gray" ,cex=2.3,inset=c(0.01,0.01))
# x.at <- c(-10,150)
