...
class picking():
    ...
    def inputResult(self, num, sac_file_name, sac1, sac2, sacZ):
        ...
        if check == "Y" or check == "y":
            ...
            logger.info(f"Result : {check}")
            self.result[num] = [sac_file_name, "z", round(sac1.t4,2), round(sac1.t1) , round(sac1.t2), round(sac1.t3)]
            self.zory = "z"
        elif check == "1":
    ...
    def mainProcess(self, sac_file_names, num):
        ...
        try:
            ...
            if not file_exists:
                ...
                result.to_csv(f"{self.asc_path}/result.csv",header=False,index=True,mode='a')
