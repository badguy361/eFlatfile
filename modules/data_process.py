...
class SACProcess():
    ...
    def removeInstrumentResponse(self, sac_file, instrument):
        s = f"r {self.sac_path}/rowdata/{sac_file} \n"
        s += "rmean; rtrend \n"
        s += "taper \n"
        s += f"trans from polezero s {instrument} to acc freq 0.02 0.1 1 10 \n"
        s += f"w {self.sac_path}/{sac_file} \n"
...
class catalogProcess():
    ...
    def addMw(self, path, catalog_name):
        self.catalog["Mw"] = np.where(self.catalog["ML"] > 6.0, 
...
if __name__ == '__main__':
    ...
    #     ]
    # for sta in stations:
    #     sac_files = sac_process.readSACFile(sta)
    #     instrument_files = sac_process.readInstrumentFile(sta)
    #     sac_process.removeInstrumentResponse(sac_files, instrument_files)
